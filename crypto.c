						  ...
						  int *out_blob_len)
{
		...
		ciphertext = wdmalloc(max_ct_len);
		if (!ciphertext)
				goto error_ctx;
						  ...
						  int *out_plain_len)
{
		...
		plaintext = wdmalloc(ciphertext_len + EVP_CIPHER_block_size(EVP_aes_256_cbc()));
		if (!plaintext)
				goto error_ctx;
