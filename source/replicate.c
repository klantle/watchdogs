...
static int package_parse_repo(const char *input,struct _repositories *__package_data)
{
  ...
  char parse_input[DOG_PATH_MAX*2];
  ...
  *repo_slash='\0';
  ...
  if(strlen(__package_data->user)==REPLICATE_RATE_ZERO ||
    ...
    char size_title[DOG_PATH_MAX*3];
...
void package_implementation_omp_conf(const char* config_name,const char* package_name)
{
  ...
  if(!ctx_file) {
    ...
    char* buffer=(char*)dog_malloc(fle_size+1);
    if(!buffer) {
...
void package_add_include(const char *modes,char *package_name,char *package_following)
{
  ...
  char *ct_modes=dog_malloc(fle_size+2);
  ...
  if(!n_file) { dog_free(ct_modes); return; }
...
{
  char dog_buffer_error[DOG_PATH_MAX],dependencies[DOG_PATH_MAX],
       _directive[DOG_MAX_PATH];
...
{
  char root_dir[DOG_PATH_MAX],includes[DOG_MAX_PATH],
       plugins[DOG_PATH_MAX],components[DOG_PATH_MAX],
       the_path[DOG_PATH_MAX*2],full_path[DOG_PATH_MAX],
  ...
  char **tmp_stack=dog_malloc(_ssize*sizeof(char*));
  ...
  rate_found_include=dog_sef_fdir(package_dir,"*.inc",NULL);
  #if defined(_DBG_PRINT)
  ...
  ++_sindex;
  snprintf(stack[_sindex],DOG_PATH_MAX,"%s",package_dir);
...
void dog_apply_depends(const char *depends_name,const char *depends_location)
{
  char _dependencies[DOG_PATH_MAX];
...
void dog_install_depends(const char *packages,const char *branch,const char *where)
{
  ...
  char package_name[DOG_PATH_MAX];
    ...
    if(where==NULL || where[0]=='\0')
      {
        ...
        if(!k) {
          ...
          if(locations[0]=='\0' || locations[0]=='.') {
            ...
            dog_apply_depends(package_name,fetch_pwd);
          } else {
            if(dir_exists(locations)==0) {
              dog_mkdir(locations);
            }
            init_location=strdup(locations);
            dog_apply_depends(package_name,locations);
          }
