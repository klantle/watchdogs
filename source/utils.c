...
int is_os_specific_archive(const char *filename)
{
		...
		for (k = 0; __dency_os_patterns[k] != NULL; ++k) {
			if (strstr(filename, __dency_os_patterns[k]))
				return 1;
		}
...
/*
 * is_gamemode_archive - check if filename indicates server/gamemode archive
 *
 * Identifies server-related files by name patterns. Returns 1 if the
 * filename contains any server pattern, 0 otherwise.
 */
int is_gamemode_archive(const char *filename)
{
		int k;

		for (k = 0; __dency_more_patterns[k] != NULL; ++k) {
			if (strstr(filename, __dency_more_patterns[k]))
				return 1;
		}
...
char *dency_get_assets(char **pkg_assets, int counts, const char *preferred_os)
{
		...
		for (i = 0; i < fetch_patterns; i++) {
			for (j = 0; j < counts; j++) {
				if (strstr(pkg_assets[j], __dency_os_patterns[i]))
		...
		for (i = 0; i < counts; i++) {
			...
			for (j = 0; j < fetch_patterns; j++) {
				if (strstr(pkg_assets[i], __dency_os_patterns[j])) {
					has_os_pattern = 1;
					break;
				}
			}
