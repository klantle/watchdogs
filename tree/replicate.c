...
package_parse_repo(const char *input, struct _repositories *pkg_dt)
{
	...
	if (strstr(path_ptr, "github.com")) {
		...
		printf("\n" FCOLOUR_BCYAN "Host not detected clearly for: %s"
		    FCOLOUR_DEFAULT "\n", input);
		printf(FCOLOUR_BCYAN "1. GitHub\n2. GitLab\n3. Gitea\n4. SourceForge\n");
    ...
    size_t put_size, const char *branch)
{
	...
	if (kevlar_repos->tag[0] &&
		...
		if (package_gh_latest_tag(kevlar_repos->user,
			...
			pr_info(stdout,
			    "Creating latest/newer tag: " FCOLOUR_CYAN
			    "%s " FCOLOUR_DEFAULT "~instead of latest " 
			    FCOLOUR_CYAN "(?newer)" FCOLOUR_DEFAULT "\t\t" 
			    FCOLOUR_YELLOW "[V]",
			    package_actual_tag);
		} else {
			pr_error(stdout,
		...
		if (asset_counts > 0) {
			...
			if (package_best_asset) {
				...
				pr_info(stdout,
				    ...
				    FCOLOUR_YELLOW "\033[1m > \033[0m" 
				    FCOLOUR_CYAN "%s\t\t" FCOLOUR_YELLOW "[V]\n",
				    package_best_asset);
    ...
    const char *plugin_name)
{
	...
	pr_color(stdout, FCOLOUR_GREEN,
	    "Create Dependencies '%s' into '%s'\t\t" FCOLOUR_YELLOW "[V]\n",
    ...
    if (found) {
        for (i = REPLICATE_RATE_ZERO; i < dogconfig.dog_sef_count; ++i) {
            ...
            if (dump_place[0] != '\0') {
                ...
                snprintf(dest_path, sizeof(dest_path), "%s%splugins%s%s",
                         dump_loc, separator, separator, package_names);
                
                char plugins_dir[DOG_PATH_MAX];
                snprintf(plugins_dir, sizeof(plugins_dir), "%s%splugins",
                         dump_loc, separator);
...
package_move_files(const char *package_dir, const char *package_loc)
{
    ...
    if (rate_found_include > 0) {
        for (i = REPLICATE_RATE_ZERO; i < dogconfig.dog_sef_count; ++i) {
            ...
            char include_dest[DOG_PATH_MAX];
...
#ifdef DOG_WINDOWS
    char cmd[DOG_PATH_MAX * 2];
    snprintf(cmd, sizeof(cmd), "rmdir /S /Q \"%s\"", package_dir);
    system(cmd);
#else
    char cmd[DOG_PATH_MAX * 2];
