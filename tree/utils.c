...
dog_procure_pwd(void)
{
	...
	if (dog_work_dir[0] == '\0') {
		...
		size_len = readlink("/proc/self/cwd", dog_work_dir,
		    sizeof(dog_work_dir) - 1);
		if (size_len < 0)
			dog_work_dir[0] = '\0';
		else
			dog_work_dir[size_len] = '\0';
#endif
	}
	return (dog_work_dir);
}
...
dog_toml_configs(void)
{
	...
	memset(include_aio_path,
	__compiler_rate_zero, sizeof(include_aio_path));
