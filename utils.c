...
/**
 * Structure to map a compiler error/warning trigger string to a detailed explanation.
 * - cs_t: A key phrase from the compiler's error message.
 * - cs_i: A human-readable explanation of the error and how to potentially fix it.
 */
typedef struct {
        char *cs_t;
        char *cs_i;
...
/**
 * An array of common compiler errors and warnings, with detailed explanations.
 * The array is terminated by a sentinel value {NULL, NULL}.
 */
...
/**
 * find_warning_error - Look up an error/warning explanation from compiler output
 ...
 * @pawn_output: Compiler output file name
 *
 * Marks lines containing errors or warnings and prints explanations from ccs
 */
void
print_file_with_annotations(const char *pawn_output)
{
        FILE *pf;
        char buffer[MAX_PATH];        /* Buffer to read each line */
        int wcnt = 0;
        int ecnt = 0;
        ...
        /* Read pf line by line until EOF */
        ...
        snprintf(__sz_compiler, 256, "COMPILE COMPLETE | WITH %d ERROR | %d WARNING", ecnt, wcnt);
